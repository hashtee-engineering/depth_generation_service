...
@click.option('--share', is_flag=True, help='Create a public Gradio share link (don’t use on RunPod)')
@click.option('--pretrained', 'pretrained_model_name_or_path', default=None, help='HF repo or local path')
@click.option('--version', 'model_version', default='v2', help='v1 or v2')
@click.option('--port', default=7860, type=int, help='Local port')
@click.option('--no-fp16', is_flag=True, help='Disable FP16 (use only if OOM)')
def main(share: bool, pretrained_model_name_or_path: str, model_version: str, port: int, no_fp16: bool):
...
''')
    ...
    print(f"\nLaunching MoGe demo locally...")
    print(f"   Local URL: http://localhost:{port}")
    print(f"   Network URL (for VS Code port forward): http://0.0.0.0:{port}\n")

    demo.launch(
        server_name="0.0.0.0",   # Critical: allows external connections (port forwarding)
        server_port=port,        # Your chosen port
        share=share,             # Only use --share locally if you want public link
        debug=True,              # Helps see errors
        quiet=False
    )
